...
export default function SitesTable({ sites }: SitesTableProps) {
	...
	const filteredAndSortedSites = React.useMemo(() => {
		try {
			...
			const filtered = searchTerm
				? sites.filter(site => {
					...
					return (
						...
						site.reason.toLowerCase().includes(searchLower) ||
						(site.isPonzi ? 'ponzi' : 'no ponzi').includes(searchLower) ||
			...
			console.error('Error al filtrar y ordenar los sitios:', error)
			return sites
		}
	}, [sites, searchTerm, sortField, sortOrder])
			...
			<div className="rounded-xl border border-gray-200 overflow-hidden bg-white">
				{filteredAndSortedSites.length === 0 ? (
					<div className="p-8 text-center">
						<p className="text-gray-500">No se encontraron sitios que coincidan con tu búsqueda.</p>
					</div>
				) : (
